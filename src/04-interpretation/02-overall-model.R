...
location_only_tva <- loadRData("./outputs/14_04_2023/outputs/overall_models/location_only_tva.rda")

# get_variables(location_only_tva)[1:10]
...
#MCMC Pair plots
draws <- as_draws_array(location_only_tva)
mcmc_scatter <- mcmc_pairs(draws,pars = as.character(params_list),off_diag_fun = "hex")
ggsave(filename = "outputs/overall_model_results/location_only_tva/mcmc_scatter.png",
...
estimate_plot <- estimates_plot(draws_df = draws_df,params_list = params_list,
                                title="Estimates for Location Only TVA Model"
)
ggsave(filename = "outputs/overall_model_results/location_only_tva/location_estimates.png",
       plot = estimate_plot,width = 1800,height=1200,units = "px")


# VPC Estimates
vpcs <- vpc(location_only_tva,as.character(params_list))
vpc_estimates <- estimates_plot(draws_df = vpcs,params_list = params_list,
                                title="VPCs for Location Only TVA Model"
)
ggsave(filename = "outputs/overall_model_results/location_only_tva/location_vpcs.png",
